...
homerange %>% 
  ...
  geom_histogram(alpha=0.2, color = "black", fill = "magenta", bins=40)+ #alpha adds transparency to colors
...
homerange %>% 
  ...
  geom_histogram(color = "black", fill = "turquoise", bins = 30, alpha = 0.6) +
  labs(title = "Distrubition of log10.hra")
```
...
homerange <- homerange %>% 
  mutate(range_category= case_when(log10.hra < 3.653 ~ "very small",
                                  log10.hra >=3.653 & log10.hra <= 4.59 ~ "small",
...
homerange %>% 
  ...
  geom_bar(position = "dodge", color="black") #if you don't want a stacked barplot put dodge
```
