...
```{r}
display.brewer.pal(8,"RdPu")
# for colorblind safe display.brewer.pal(8, "Paired")
```
...
```{r}
my_palette <- paletteer_d("colRoz::sky")
```
...
homerange %>% 
  ...
  geom_point(na.rm = T) +
  xlim(0, 4) + #zooms in on the plot, focuses on areas between the variables
  ylim(1, 6)
```
...
```
```{r}
p2+scale_color_brewer(palette = "RdPu")
```
...
ecosphere %>% 
  ggplot(aes(x=diet, y=log10_mass, fill=migratory_strategy))+ 
  geom_boxplot(alpha=0.4)+ 
  facet_grid(migratory_strategy~.)+ #~. after is row, before is column
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  labs(title = "Mass by Diet and Migratory Strategy",
       x = NULL,
       y = "Log 10 Mass",
