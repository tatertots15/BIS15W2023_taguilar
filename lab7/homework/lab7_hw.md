...
```r
amniota %>% 
  summarize(number_nas = sum(is.na(amniota))) #as the NA's are not represented by NA they do not appear so these values do not make sense.
```
...
```r
amphibio %>% 
  summarize(number_nas = sum(is.na(amphibio))) #since the NA's are represented by NA they appear.
```
...
amphibio %>% 
  ...
  summarise(total_NA=sum(is.na(Ter)),
            total_NA=sum(is.na(Aqu)),
            total_NA=sum(is.na(Arb)),
            total_NA=sum(is.na(Fos))) #I think that these are meant to be NA as maybe the species doesn't burrow but can live in water. 
```
