...
```{r}
ecosphere %>% 
  count(habitat, common_name, sort=T) %>% 
  tabyl(habitat)
```
...
```{r}
anyNA(conservation_eagles$population_size)
```
...
I wanted to see if the life expectancy of hummingbirds affected whether or not they laid more eggs per clutch on average. Hummingbirds that lived longer tended to lay more eggs on average.

```{r}
hummingbird <- ecosphere %>% 
  filter(family == "Trochilidae") %>% 
  select(common_name, scientific_name, life_expectancy, mean_eggs_per_clutch)
hummingbird
```
...
hummingbird %>% 
  ...
  summarize(mean(mean_eggs_per_clutch),
            total=n())
```
