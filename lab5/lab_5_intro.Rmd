...
```{r}
bison_new <- select(bison, animal_sex, animal_weight, animal_yob)
```

4. Pull out the animals born between 1980-1990.

```{r}
filtered_bison <- filter(bison_new, animal_yob>= 1980 & animal_yob<=1990)
```

5. How many male and female bison are represented between 1980-1990?

```{r}
table(filtered_bison$animal_sex)
```

6. Between 1980-1990, were males or females larger on average?

```{r}
bison_m <- filter(filtered_bison, animal_sex=="M")
```
...
```{r}
bison_f <- filter(filtered_bison, animal_sex=="F")
```
