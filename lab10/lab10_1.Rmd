...
```{r}
life_history %>% 
  count(order, sort=T) %>% #sort=T is another way to arrange T is numerical 
...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) #without x or y
```
```{r}
...
elephants %>% 
  ...
  labs(title = "Elephant Age vs. Height",
       x = "Age",
       y = "Height") +
  theme(plot.title=element_text(size=rel(1.5), hjust =.5)) #size=rel adjusts the size of the title
```
